...
def plot_clustering_coefficient(clustering_coeff, bins=None, show=False, fig_nr=None, network_name=None, legend_on=True, normalized=False, line_type=None, see_through=True):
    ...
    if normalized:
        ...
        plt.yscale("log")
        if network_name is not None:
            plt.hist(clustering_coeff,bins,ls=line_type, lw=3,label=network_name,edgecolor=color,fc=color,alpha=alpha)
...
if __name__=="__main__":
    ...
    for i,network in enumerate(  network_paths ):
        ...
        plot_spl_hist(spl,fig_nr=i+4, network_name=network_names[i])
        # if save_plots:
        ...
        # if save_plots:
            # save_plot(network_names[i].replace(" ", "_").lower()+"_degree_dist_v","../report/figs/task1/")
        plot_clustering_coefficient(clustering_coeff, fig_nr=i+1, network_name=f"{network_names[i]} random")
        if save_plots:
            save_plot(network_names[i].replace(" ", "_").lower()+f"_clustering_coeff_w_{nr_random_networks}_rand_v","../report/figs/task2/")
        plot_spl_hist(spl,fig_nr=i+4, network_name=f"{network_names[i]} random",normalized=True)
        if save_plots:
